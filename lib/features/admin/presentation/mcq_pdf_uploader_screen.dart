...
class McqPdfUploaderScreen extends ConsumerStatefulWidget {
  const McqPdfUploaderScreen({super.key});

  @override
  ConsumerState<McqPdfUploaderScreen> createState() => _McqPdfUploaderScreenState();
}

class _McqPdfUploaderScreenState extends ConsumerState<McqPdfUploaderScreen> {
  ...
  Future<void> _upload() async {
    ...
    try {
      ...
      final storageRef = FirebaseStorage.instance.ref().child('mcq_pdfs/$_selectedSubjectId/$fileName');
      ...
      await FirebaseFirestore.instance.collection('mcq_pdfs').add({
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('MCQ PDF uploaded successfully!')));
  ...
  Widget build(BuildContext context) {
    // We want all subjects including aptitude/verbal for the admin
    final subjectsAsync = ref.watch(placementSubjectsProvider);

    return Scaffold(
      appBar: AppBar(title: const Text('Upload MCQ PDF')),
      body: subjectsAsync.when(
        data: (subjects) {
          // Manually add Aptitude and Verbal if they are missing due to the filter
          final List<Map<String, String>> allSubjects = [
            {'id': 'aptitude', 'name': 'Aptitude'},
            {'id': 'verbal', 'name': 'Verbal Review'},
            ...
            child: ListView(
              ...
              children: [
                DropdownButtonFormField<String>(
                  ...
                  items: allSubjects.map((s) => DropdownMenuItem(value: s['id'], child: Text(s['name']!))).toList(),
                  ...
                  child: Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        Text(_selectedFile == null ? 'Select MCQ PDF' : _selectedFile!.name, textAlign: TextAlign.center),
                      ],
                    ),
                  ),
                ),
                ...
                const SizedBox(height: 16),
                ElevatedButton(onPressed: _isUploading ? null : _upload, child: const Text('Upload MCQ PDF')),
              ],
            ),
          );
        },
